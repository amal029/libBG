...
* Quick example
  ...
  - We use the [[https://people.sc.fsu.edu/~jburkardt/cpp_src/rkf45/rkf45.html][RK45]] integrator along with the ~Solver~ class that
    ...
    #+begin_src c++
      ...
      // The main function computing the slope
      void toIntegrate(double t, double x[], double dxdt[]) {

        size_t N = gs->getComponentSize(); // number of equations
        std::memmove(xv.data(), x, N * sizeof(double)); // moving from array to vector
        std::vector<double> res;                            
        gs->dxdt(xv, res);                                  // getting the derivative via solver.
        std::memmove(dxdt, res.data(), N * sizeof(double)); // getting the result back into array
      }
      ...
      void integrate(Solver<double> &s) {
        ...
        xv.reserve(neqn);		// number of equations

        // This is heap allocated to avoid VLA
        double *Y = new double[neqn];
        double *YP = new double[neqn];
        double t = 0; // The current value of time.
        double tout = 50;		// how many seconds
        double abserr = sqrt(DBL_EPSILON); // abs errors for rk45
        double relerr = sqrt(DBL_EPSILON); // relative errors for rk45
        int FLAG = 0;		     // API for rk45 call

        // Initial values for integrating the state space equations
        Y[0] = 0; Y[1] = 1; Y[2] = 2; Y[3] = 10;
        ...
        while (FLAG != 2) {		// FLAG = 2 at end of time 50 seconds.
          // Call the rk45 solver.
          FLAG = r8_rkf45(toIntegrate, neqn, Y, YP, &t, tout, &relerr, abserr, -1);
          // Write the output after each integration step
          file << t << ", " << Y[0] << "," << Y[1] << "," << Y[2] << "," << Y[3]
               << "\n";
        }
        file.close();			// close the file
        delete[] Y;			// delete the allocated arrays
        delete[] YP;
      }
    ...
    #+end_src
  - The complete example is present in tests/test2.cpp in the libBG directory.
  - The output of the running the ~test2~ is a csv file that can be
