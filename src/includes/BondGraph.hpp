...
template <size_t N = 64> struct BondGraph {
  // Public methods
  BondGraph(std::string &&name, bool Jpref = true) : name(name), Jpref(Jpref) {}
  BondGraph(const BondGraph &) = default;
  BondGraph(BondGraph &&) = default;
  BondGraph &operator=(const BondGraph &) = default;
  ...
  // Here we start adding the switches (hybrid models)
  constexpr void addSwitch(junctionVariantPtr j) {
    switch_junctions.emplace_back(j);
  }

  constexpr void offJunction(size_t id, ComponentType T) {
    // Get all the outgoing edges
    std::vector<size_t> &oedges = getEdges()[id];
    ...
    for (size_t i = 0; i < oedges.size(); ++i) {
      ...
      if (T == ComponentType::J0) {
        j = addComponent(Component<ComponentType::SE>{"se"});
      } else if (T == ComponentType::J1) {
        j = addComponent(Component<ComponentType::SF>{"sf"});
      }
      edges[j].push_back(oedges[i]); // se/sf --> component
      // Get the position of the id in reverse edges
      std::vector<size_t> &g = redges[oedges[i]];
      auto p = std::find(g.begin(), g.end(), id);
      assert(p != g.end() && "Component not connected to switch");
      *p = j;
    }
    std::vector<size_t> &iedges = getRedges()[id];
    ...
    for (size_t i = 0; i < iedges.size(); ++i) {
      ...
      edges[j].push_back(iedges[i]); // se/sf --> component
      std::vector<size_t> &g = edges[iedges[i]];
      auto p = std::find(g.begin(), g.end(), id);
      assert(p != g.end() && "Component not connected to switch");
      *p = j;
    }
  ...
  // se:0 or sf:0
  template <ComponentType J0, Modulated M>
  constexpr void offJunction(Component<ComponentType::J0, M> &junc) {
    ...
    junc.setDeleted(); // This junction is now gone.
  }

  template <ComponentType J1, Modulated M>
  ...
  void buildSwitchBondGraphs() {
    ...
    for (size_t i = 0; i < (size_t)std::exp2(switch_junctions.size()); ++i) {
      ...
      for (size_t j = 0; j < switch_junctions.size(); ++j) {
        if (vv[j] == 0) {
          Component<ComponentType::J0, Modulated::F> *junc =
...
private:
  ...
  constexpr void OutputsEqualInputs(expressionAst &space, Component<T, M> &x) {
    ...
    for (size_t i = 0; i < edges[myID].size(); ++i) {
      ...
      // Outport --> Inport
      size_t out_sym_index =
          space.append(Symbol{myOutPort->getOutCausalName()});
      size_t in_sym_index = space.append(Symbol{nInPort->getInCausalName()});
      size_t _ = space.append(Expression<EOP::EQ>{in_sym_index, out_sym_index});

      // Outport <-- Inport
      out_sym_index = space.append(Symbol{nInPort->getOutCausalName()});
      in_sym_index = space.append(Symbol{myOutPort->getInCausalName()});
  ...
  constexpr void addToSpace(expressionAst &space,
    ...
    size_t out_port_index = space.append(Symbol{outport->getOutCausalName()});
  ...
  [[nodiscard]]
  constexpr size_t addToSpaceCL(expressionAst &space, Port *inport,
                                bool isIntegral, const std::string &vstr,
                                std::string &internal) const {
    assert(inport->getPortType() == PortType::IN);
    // This has integral causality.
    size_t value_index = space.append(Symbol{vstr, true});
    size_t out_index;
    if (isIntegral) {
      size_t in_index = space.append(Symbol{inport->getInCausalName()});
      internal = "d" + std::string(inport->getOutCausalName());
      out_index = space.append(Symbol{internal});
  ...
  constexpr size_t addToSpaceTFGY(expressionAst &space, Port *inport,
    ...
    if (inport->getOutCausality() == Causality::Effort) {
      ...
      size_t res =
          space.append(Expression<EOP::MUL>(out_in_index, value_index));
      size_t res1 = space.append(Symbol{inport->getOutCausalName()});
      res1 = space.append(Expression<EOP::EQ>{res1, res});
  ...
  constexpr void addToSpaceJ0J1(expressionAst &space, Port *mainPort,
    ...
    // Handling the causality
    if (mainPort->getPortType() == PortType::OUT)
      size_t _ = space.append(Expression<EOP::EQ>{rres, res});
    else {
      size_t num_index = space.append(Number{-1});
      res = space.append(Expression<EOP::MUL>{num_index, res});
      size_t _ = space.append(Expression<EOP::EQ>{rres, res});
    }
  ...
  std::vector<junctionVariantPtr> switch_junctions;
  std::vector<BondGraph<N>> switched_bond_graphs;
  std::vector<std::bitset<N>> switches;
...
static std::ostream &operator<<(std::ostream &os, BondGraph<N> &g) {
  // First get all the sources
  std::vector<size_t> sources;
